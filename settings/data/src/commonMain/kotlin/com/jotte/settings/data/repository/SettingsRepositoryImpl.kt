...
internal class SettingsRepositoryImpl(private val datastore: DataStore<Preferences>) :
    ...
    override fun readAppAppearance(): Flow<AppAppearance?> {
        return readValue(AppearanceKey, null).map { value -> value?.let(AppAppearance::valueOf) }
    }

    override fun readSoundEffects(): Flow<Boolean?> {
        return readValue(SoundEffectsKey, null)
    }
}